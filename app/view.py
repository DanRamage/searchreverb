...
class BaseAPI(MethodView):
    ...
    def json_error_response(self, error_code, error_message):
        json_error = {}
        json_error["error"] = {"code": error_code, "message": error_message}
        return json_error
    ...
    def index(self):
        ...
        if not login.current_user.is_authenticated:
            current_app.logger.error(
                "IP: %s not logged in attempting to access page" % (request.remote_addr)
            )
        ...
        try:
            ...
            rendered_template = self.render(
                "search_page.html", categories=json_results["categories"]
            )

        except Exception as e:
            current_app.logger.exception(e)
        current_app.logger.debug(
            "dispatch_request finished in %f seconds" % (time.time() - start_time)
        )
...
class AddSearchItem(MethodView):
    ...
    def post(self):
        ...
        try:
            ...
            db.session.commit()
            ret_code = 200
            ret_val = {"status": 200, "message": "Search added successfully."}
        except IntegrityError as e:
            current_app.logger.exception(e)
            ret_code = 500
            ret_val = {"status": 500, "message": "Search already exists"}
        except Exception as e:
            current_app.logger.exception(e)
            ret_code = 500
            ret_val = {"status": 500, "message": "Error adding search."}
...
class base_view(sqla.ModelView):
    ...
    def on_model_change(self, form, model, is_created):
        start_time = time.time()
        current_app.logger.debug(
            "IP: %s User: %s on_model_change started"
        ...
        current_app.logger.debug(
            "IP: %s User: %s on_model_change finished in %f seconds"
...
class AdminUserModelView(base_view):
    ...
    form_extra_fields = {"password": fields.PasswordField("Password")}
    column_list = (
        "login",
        "first_name",
        "last_name",
    ...
    def on_model_change(self, form, model, is_created):
        ...
        start_time = time.time()
        current_app.logger.debug(
            "IP: %s User: %s AdminUserModelView on_model_change started."
            % (request.remote_addr, current_user.login)
        )
        ...
        current_app.logger.debug(
            "IP: %s User: %s AdminUserModelView create_model finished in %f seconds."
            % (request.remote_addr, current_user.login, time.time() - start_time)
        )
...
class BasicUserModelView(AdminUserModelView):
    ...
    def get_query(self):
        ...
        return (
            super(AdminUserModelView, self)
            .get_query()
            .filter(User.login == login.current_user.login)
        )
        ...
        if current_user.is_active and current_user.is_authenticated:
            ...
            if current_user.has_role("admin"):
                ...
                return self.session.query(self.model).filter(
                    self.model.login == current_user.login
                )
    ...
    def get_count_query(self):
        ...
        if current_user.is_active and current_user.is_authenticated:
            ...
            if not current_user.has_role("admin"):
                cnt = self.session.query(func.count("*")).filter(
                    self.model.login == current_user.login
                )
            else:
...
class MyAdminIndexView(admin.AdminIndexView):
    ...
    def index(self):
        ...
        if not login.current_user.is_authenticated:
            current_app.logger.debug(
                "IP: %s User: %s is not authenticated"
                % (request.remote_addr, login.current_user)
            )
    ...
    def login_view(self):
        ...
        if helpers.validate_form_on_submit(form):
            ...
            current_app.logger.debug(
                "IP: %s User: %s is not authenticated"
                % (request.remote_addr, form.login.data)
            )
    ...
    def register_view(self):
        ...
        if helpers.validate_form_on_submit(form):
            try:
                ...
                role_map = Roles_Users(user_id=user.id, role_id=role.id)
...
class SearchItemView(sqla.ModelView):
    ...
    def delete_model(self, model):
        ...
        current_app.logger.debug(
            "IP: %s Deleting search: %s(%d)"
            % (request.remote_addr, model.search_item, model.id)
        )
        search_results_deleted = -1
        try:
            recs = (
                self.session.query(SearchResults)
            ...
            current_app.logger.error(
                "Error when deleting search results for search id: %d" % (model.id)
            )
            current_app.logger.exception(e)
            self.session.rollback()
        try:
        ...
        current_app.logger.debug(
            "Deleted search: %s(%d) and %d search results"
            % (model.search_item, model.id, search_results_deleted)
        )
        return True
  ...
  """
    ...
    def get_query(self):
        if not current_user.has_role("admin"):
            return (
                super(SearchItemView, self)
                .get_query()
                .filter(SearchItem.user_id == login.current_user.id)
            )
        else:
    ...
    def get_count_query(self):
        ...
        if current_user.is_active and current_user.is_authenticated:
            ...
            if not current_user.has_role("admin"):
                cnt = self.session.query(func.count("*")).filter(
                    SearchItem.user_id == login.current_user.id
                )
            else:
                cnt = super().get_count_query()
        return cnt
...
class reverb_search(reverb_listings_rest):
    def get(self):
        current_app.logger.debug(
            "IP: %s reverb_search. Params: %s" % (request.remote_addr, request.args)
        )
