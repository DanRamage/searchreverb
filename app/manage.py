...
def add_user(params):
  ...
  try:
    ...
    for site in search_sites:
      search_site = SearchSite(row_entry_date=row_entry_date,
                               site_name=site)
  ...
  current_app.logger.debug("add_search_sites finished in %f seconds" % (time.time()-start_time))
...
def run_gc_searches(params):
  ...
  try:
    ...
    for user in users:
      ...
      search_results = []
      for search_rec in search_recs:
        current_app.logger.debug("Running query for Email: %s Query params: %s" % (user.email, search_rec))

        listings = search_obj.search_used(search_rec.search_item, search_rec.min_price, search_rec.max_price, search_site_rec.id)
        if len(listings):
          results_to_report = process_normalized_results(user, search_rec, listings)
