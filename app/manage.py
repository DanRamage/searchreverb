...
def process_results(user_rec, search_rec, listings):
    try:
        ...
        if len(prev_search_results):
            ...
            for prev_search_result in prev_search_results:
                ...
                try:
                    ...
                    if listing:
                        if (
                            float(listing["price"]["amount"])
                            != prev_search_result.last_price
                        ):
                            current_app.logger.debug(
                                "Price change detected for item: %d From: %s to %s"
                                % (
                            ...
                            try:
                                ...
                                db.session.rollback()
                                current_app.logger.error(
                                    "Error updating the search record: %d price."
                                    % (search_rec.id)
                                )
                                current_app.logger.exception(e)
        ...
        if not search_rec.show_new_results_only:
            ...
            results_to_report = [
                listing for listing in listings if listing["id"] in new_results
            ]
        ...
        if len(new_results):
            ...
            for new_result_id in new_results:
                try:
                    current_app.logger.debug(
                        "New results %d for user: %s search: %s(%d)"
                        % (
                            new_result_id,
                            user_rec.email,
            ...
            current_app.logger.debug(
                "No new results for user: %s search: %s(%d)"
        ...
        if len(deleted_search_results):
            for deleted_results in deleted_search_results:
                current_app.logger.debug(
                    "Search item: %d is no longer in the current results, removing from db"
                    % (deleted_results)
                )
                try:
                    db.session.query(SearchResults).filter(
                        SearchResults.search_item_id == deleted_results
                    ).delete()
                    db.session.commit()
                except Exception as e:
                    db.session.rollback()
        ...
        if len(new_results):
            ...
            for new_result_id in new_results:
                try:
                    ...
                    # Find the list record based on id so we can get the price.
                    listing_rec = next(
                        listing for listing in listings if new_result_id == listing.id
                    )
                    new_result = NormalizedSearchResults(
                        ...
                        search_site_id=listing.search_site_id,
                    )
                    db.session.add(new_result)
                    db.session.commit()
                except IntegrityError as e:
            ...
            for deleted_results in deleted_search_results:
                ...
                try:
                    db.session.query(NormalizedSearchResults).filter(
                        NormalizedSearchResults.search_item_id == deleted_results
                    ).delete()
                    db.session.commit()
                except Exception as e:
                    db.session.rollback()
        ...
        try:
            ...
            try:
                ...
                out_file = os.path.join(
                    app.root_path, "%s/%s.html" % (RESULTS_TEMP_DIRECTORY, filename)
                )
...
def output_normalized_results(app, search_results, user, email_results):
    # def output_results(app, results, user, search_rec, email_results):
    run_time = datetime.now()
    file_attach_list = []
    for search_rec, results in search_results:
